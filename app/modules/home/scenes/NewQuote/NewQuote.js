...
class NewQuote extends React.Component {
    ...
    refreshButton(text, color){
        const showButton = (text.trim().length > 0) || color;
        const edit = (this.props.edit); //check if in edit mode
        let data = {text, color : color ? color : this.state.color, edit}
    ...
    onChangeText(text) {
        this.setState({text});
        this.refreshButton(text, null);

    }

    onSelectColor(color) {
        this.setState({color});
        this.refreshButton(this.state.text, color);
